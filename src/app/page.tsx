...
export default function Home() {
  ...
  const cleanAndParse = (value: string | number) => {
    if (typeof value === 'string') {
      //Le quito caracteres especiales al string $ , ( ) 
      const cleanedValue = value.replace(/[$,()]/g, '');
      //Si no se pudo convertir devuelvo un -1 como bandera
      return parseFloat(cleanedValue) || -1;
    } else {
      return value;
    }
  };
            ...
            {orderedData.map((data, index) => {
              ...
              return (
                <tr key={`${data['Company Name']}-${index}`}>
                  <td className='px-3 max-w-[250px] overflow-hidden text-ellipsis'>{data['Company Name']}</td>
